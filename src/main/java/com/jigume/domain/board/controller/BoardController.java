...
public class BoardController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "게시판 권한이 없음. (판매자나 주문자가 아님)", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AuthNotAuthorizationMemberException.class))),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "게시판을 가져온다.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = BoardDto.class))),
            @ApiResponse(responseCode = "401", description = "게시판 권한이 없음.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AuthNotAuthorizationMemberException.class))),
            @ApiResponse(responseCode = "404", description = "회원을 찾을 수 없음.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AuthMemberNotFoundException.class))),
            @ApiResponse(responseCode = "404", description = "보드를 찾을 수 없음.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = BoardNotFoundException.class)))
    })
    @PostMapping("/{boardId}")
